...
function parse(input) {
    ...
    function date(day, month, year) {
        if (day === null) {
            ...
            if (month === 1) return day+" Januari "+year;
            else if (month === 2) return day+" Februari "+year;
            else if (month === 3) return day+" Maret "+year;
            else if (month === 4) return day+" April "+year;
            else if (month === 5) return day+" Mei "+year;
            else if (month === 6) return day+" Juni "+year;
            else if (month === 7) return day+" Juli "+year;
            else if (month === 8) return day+" Agustus "+year;
            else if (month === 9) return day+" September "+year;
    ...
    function lex(raw) {
        ...
        const synt_image        = /^[ ]*img\[([\w.,"'/\\ ]+)][ ]*\(([\w,. )(</>]+)\)[ ]*(xs|sm|m|lg|xl)?[ ]*$/m;
        const synt_table_head   = /^[ ]*\|([\w,. )(</>|]+)\|[ ]*\(([\w,. )(</>]+)\)[ ]*(xs|sm|m|lg|xl)?[ ]*$/m;
        ...
        for (let i = 0; i < raw.length; i++) {
            if ((result = synt_meta.exec(raw[i])) != null) {
                ...
                if (result[1] === '1. ') {
                    ...
                    while (i+1 < raw.length && (result = /^[ ]*1\. [ ]*([\w,. )(</>]+)$/m.exec(raw[i+1])) != null) {
                ...
                markup = '<div id="##REP-ID##" class="ls-lampiran">'
                    + '<div class="ls-lampiran-logo"><img src="img/title-logo.png" alt=""></div>'
    ...
    function construct(memory) {
        ...
        if (pattern.length > 1) {
                ...
                if (operand === '1') {
                    ...
                    if (on_bab === 1) {
                        ...
                        toc_content = toc_content
                            ...
                            + '<li><span class="di-b">bab v</span><a href="#bab-i" class="con-idx"></a></li>';
                        img_table = img_table
                    ...
                    if (section === 1) {
                        const id        = img_id++;
                        img_table       += '<li class="df-tabel"><span>' +value+ '</span><a href="#i' +id+ '"></a></li>';
            ...
            toc_opening = toc_opening + temp_img_table + temp_tbl_table + temp_att_table;
            const toc =
                '<div id="pg-daftar_isi"><div class="pendahuluan-sub-title">daftar isi</div><ul>' +
...
function factory(type, data) {
    ...
    function cover(data) {
        return '<div id="cover">'
            ...
            + '<div><img src="img/title-logo.png" alt=""></div><div>'
            + 'oleh :<br>'
            + '<span>'+data[0]+'</span><br>'
            + '<span>'+data[1]+'</span><br>'
            + '<span>bidang minat</span><br>'+data[4]+'<br></div><div id="cov-footer">'
            + 'jurusan ' +data[2]+ '<br>fakultas ' +data[5]+ '<br>' +data[6]+ '<br>'
            + data[7]+ '</div></div>';
    }

    function kata_pengantar(data) {
        return '<div id="pg-kata_pengantar"><div id="pg-kata_pengantar-content">'
            + '<div class="pendahuluan-sub-title">kata pengantar'
            + '<img src="img/basmallah.jpg" alt=""></div><p>'
